...
| “Daily resets at the time displayed.” | ✅ Implemented: refill now uses `elapsed >= window` (exact-boundary charge refills). Intervals are still anchored at save time. | Use `>=` at the boundary and test exact-boundary behavior; show the actual next reset time. Do not call it a rolling window. |
