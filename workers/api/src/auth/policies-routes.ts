...
import { hasDeviceLink } from "@/auth/device-db";
import {
...
policyRoutes.get("/policies/:phygitalToken/approvals", async (c) => {
  ...
  // Inbox is D1-only: trust the device link index (no DO isOwner round-trip).
  if (!(await hasDeviceLink(session.credentialId, phygitalToken))) {
    return json({ approvals: [] });
  }

  const approvals = await listOpenApprovals(phygitalToken);
  return json({ approvals });
});
